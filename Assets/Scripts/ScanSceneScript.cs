    ...
    void RefreshScrollViewContentScan()
    {
        ...
        if (scanElementsCount < scannedDevices)
        {
            ...
            for (int i = scanElementsCount; i < scannedDevices; i++)
            {
                ...
                ScanElementRect.position = position.position;
                Debug.Log("ScanElementRect position = " + ScanElementRect.position + " scanElementHeight " + scanElementHeight + " Amount Scan elements " + i);
                ScanElements.Add(ScanElementClone);
                //Try
                scrollViewContentScan.position = position.position;
                Debug.Log("ScanElement in scrollView "+ scrollViewContentScan.position);
            }
            scrollViewContentScan.sizeDelta = new Vector2(0, scanElementHeight * scannedDevices);
            Debug.Log("ScanElement Canvas " + scrollViewContentScan.position + " scanElementHeight " + scanElementHeight + " scanElementcount "+ scannedDevices);
